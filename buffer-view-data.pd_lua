...
function C:metadata(_,a)
    ...
    if self.kind=='sample' and ready==1 and self.pending then
        ...
        self.tempo_bpm,self.tempo_authority=nil,nil
...
function C:request(sel,a)
    ...
    if sel=='tempo-set' then
        if self.kind~='sample' or self.ready~=1 or not finite(a[2]) or a[2]<1 or a[2]>999 then return end
        if a[3]~='manual' and a[3]~='beats' and a[3]~='rendered' then return end
        self.tempo_bpm,self.tempo_authority=a[2],a[3]
        self:publish()
        pd.send(a[1],'tempo',{self.tempo_bpm,self.tempo_authority});return
    elseif sel=='tempo' then
        pd.send(a[1],'tempo',{self.tempo_bpm or 0,self.tempo_authority or 'unknown'});return
    end
    if sel=='vacant' then
