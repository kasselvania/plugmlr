...
check('cells 0 16',['cancel','region 1 11'])
check('cells 2 6',['cancel','region 2.25 4.75'])
check('cells 15 16',['cancel','region 10.375 11'])
...
p=Path('docs/evidence/grid-loop');p.mkdir(exist_ok=True,parents=True);(p/'native-regions.json').write_text(json.dumps(results,indent=2)+'\n')
