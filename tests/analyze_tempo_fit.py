...
def inspect(r,y,windows,starts):
 ...
 for pos,gain,dsp in [(4,5,8),(6,7,9)]:
  ...
  # sig~ exposes the rate at a block edge, while a logical-time motion request
  # can occur inside that block. The passive reader tap adds another block.
  # Bound by the preceding two blocks of actual captured rate, not just 64 samples.
  recent=np.lib.stride_tricks.sliding_window_view(np.pad(abs(r[:,3]),(128,0),mode="edge"),129).max(axis=1)
  limit=np.maximum(recent[:-1],recent[1:])*FILE/HOST+.08
 ...
 gain=float(np.sum(x[round(.2*HOST):round(.9*HOST)]*m[round(.2*HOST):round(.9*HOST)])/max(np.sum(x[round(.2*HOST):round(.9*HOST)]**2),1e-20))
...
def analyze(folder):
 ...
 results['core']=a
 checks['core_bounded_finite']=a['finite'] and all(HOST*12-1024<=n<=HOST*12+64 for n in a['frames'])
 ...
 results['rates']=cases
 w=r[round(5.03*HOST):round(5.99*HOST),2];checks['partial_loop_uses_same_rate_and_requested_bounds']=bool(w.min()>=.5*FILE-1 and w.max()<=1.5*FILE+1)
 ...
 if (folder/'stress-master.wav').exists():
  ...
  results['stress']=a
  checks['stress_bounded_finite']=a['finite'] and all(HOST*12-1024<=n<=HOST*12+64 for n in a['frames'])
  ...
  checks['stress_all_transition_steps_bounded']=max(a['player_max_adjacent_step_lr'])<.025*48000/HOST
 ...
 if (folder/'musical-master.wav').exists():
  ...
  checks['musical_finite_bounded_stopped']=bool(np.isfinite(r).all() and np.isfinite(y).all() and HOST*12-1024<=n<=HOST*12+64 and np.max(abs(y[round(11.03*HOST):]))==0)
 ...
 return {'host_hz':HOST,'file_hz':FILE,'checks':checks,'passed':all(checks.values()),'results':results,'limits':['Numerical checks are not listening acceptance','Applies only to the named host/file rates and captures','No universal click-free or phase-lock claim']}
if __name__=='__main__':
 parser=argparse.ArgumentParser(description=__doc__)
 parser.add_argument('folder',type=Path)
 parser.add_argument('--host-hz',type=int,default=48000)
 parser.add_argument('--file-hz',type=int,default=44100)
 args=parser.parse_args();HOST=args.host_hz;FILE=args.file_hz
 result=analyze(args.folder);print(json.dumps(result,indent=2));sys.exit(not result['passed'])
