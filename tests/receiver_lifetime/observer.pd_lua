...
function C:initialize(_,a) self.inlets,self.outlets=0,0;self.seq=0;self.receivers={};self.forward=tonumber(a[1])==1;return true end
...
function C:observed(kind,a)
 self:log(kind,a)
 if self.forward then pd.send('rr-observation',kind,a)end
end
function C:postinitialize()
 ...
 bind('rr-track',function(s,_,a)s:observed('track',a)end)
 for slot=1,16 do
  bind(slot..'-sample-loaded',function(s)s:observed('loaded',{slot});pd.send('rr-query','list',{slot})end)
  local reply='rr-owner-'..slot
  bind(reply,function(s,sel,a)local b={slot};for _,v in ipairs(a)do b[#b+1]=v end;s:observed(sel,b)end)
 end
