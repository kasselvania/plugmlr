...
for row in (1,2,6):
 for overlap in (0,5,79,80,81,160):
  for first_up in (2,7):
   ...
   if overlap>=80:want+=[event(20+overlap,'loop',row,2,8)]
...
for case in cases[boundary_cases:]:
 for command in case['commands']:command[0]*=2
 for expected in case['expected']:expected[0]*=2
class Patch:
...
for index,case in enumerate(cases):
 base=500*index;case['base_ms']=base
...
score += [(500*len(cases),'grid-hold-input','connected 0'),(500*len(cases)+5,'grid-hold-finish','bang')]
last=0;lines=[]
for t,dst,msg in sorted(score,key=lambda v:v[0]):lines.append(f'{t-last} {dst} {msg};');last=t
(OUT/'score.txt').write_text('\n'.join(lines)+'\n')
manifest=dict(base='a09c10b5a2a653862d648564f93a9ac3586656d3',production_sha256=digest,test_class=name,cases=cases,duration_ms=last,hold_ms=80,cancellation_timing_scale=2)
