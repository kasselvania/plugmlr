...
manifest=dict(score=score,base='fe0a5d49027a0eb0cb75f20d87063e34ca0f9810',production_sha256={f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in ROOT.iterdir() if f.suffix in ('.pd','.pd_lua')},fixture_changes='Unique Lua classes; original players/slots 901/902 mapped to musical tracks 1/2; private ppq/Grid/pattern/snapshot/LED buses; screen requests redirected to unused bus; passive accepted-action and state probes. No DAC, writesf, device or global DSP command.',fixture_sha256={f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in OUT.iterdir() if f.suffix in ('.pd','.pd_lua') or f.name in ('source.wav','score.txt')})
