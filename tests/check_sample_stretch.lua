...
function pd.send(n,s,a)
 ...
 else local r=receivers[n];if r then r[1][r[2]](r[1],s,a) end end
end
...
c:selection('list',{'sample_buffer_2',2,0,100,48000});local count=#sent;c:copy_loaded()
for i=count+1,#sent do assert(sent[i][1]~='1-buffer-select') end
c:command('load',{});assert(c.loaded);c:copy_loaded();assert(sent[#sent-1][1]=='1-buffer-select')
...
print('PASS: invalid input, integer argv, shell quoting, duplicate render, cancellation, occupied slots, manual selection wins, load timeout')
